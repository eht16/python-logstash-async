...
sends Logstash messages using UDP and TCP. (`SynchronousLogstashHandler` is a synchronous alternative) For example:
...
========================
The `AsynchronousLogstashHandler` and `SynchronousLogstashHandler` support also the logstash `http input plugin <https://www.elastic.co/guide/en/logstash/current/plugins-inputs-http.html>`_.
